...
export async function GET(req: NextRequest) {
  ...
  const { data: medicos } = await admin
    .from('medicos')
    .select('id, rut, nombre, apellido, especialidades(id, nombre)');
  ...
  if (!especialidad) {
    return NextResponse.json({ error: 'El médico no tiene especialidad asignada' }, { status: 404, headers: CORS });
  }

  const { data: listaEspera } = await admin
    .from('lista_espera')
    .select('id, posicion_actual_fila, gravedad, tiempo_estimado_espera_dias, pacientes:paciente_id(id, nombre, apellido_paterno, apellido_materno, rut)')
    .eq('especialidad_id', especialidad.id)
    .order('posicion_actual_fila');

  return NextResponse.json({
    medico: { nombre: medico.nombre, apellido: medico.apellido, rut: medico.rut },
